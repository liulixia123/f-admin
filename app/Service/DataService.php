            ...
            case 'types':
                switch ($kind[1]){
                    case 'add_or_update':
                        ...
                        if(!empty($inputs['picfile'])){
                            $model->picfile = upload($inputs['picfile']);
                        }
                        if(!empty($inputs['checkedpicfile'])){
                            $model->checkedpicfile = upload($inputs['checkedpicfile']);
                        }
                ...
                switch ($kind[1]){
                    case 'add_or_update':
                        ...
                        try{
                            ...
                            $types = $inputs['game_type'];
                            if (!empty($types)) {
                                foreach ($types as $k => $type) {
                                    if (empty($type)) unset($types[$k]);
                                }
                            }
                            $model->saveTypes($types);
                        }catch (\Exception $e){
                            return ['status'=>0,'msg'=>trans('fzs.common.fail')];
                        }
                        return ['status'=>1,'msg'=>trans('fzs.common.success')];
