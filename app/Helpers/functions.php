...
/**
 * [generateOperation 根据菜单生成路由，控制器，模型，模板等]
...
function generateOperation($name){
    ...
    if(strpos($str,$name)===false){
        if(substr_count($name,'/')==1){
            ...
            $exitcode = Artisan::call('make:controller '.$controllername.'Controller --resource');
            //exec('php artisan make:controller '.$controllername.'Controller --resource');
            return true;
        }else{
            return false;
        }
